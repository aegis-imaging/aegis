...
Environment variables:
    ...
    GEMINI_TOTAL_DEADLINE_SECONDS  — overall per-study deadline for the frame
                                     loop; partial results are returned with a
                                     warning once exceeded (default: 300)
"""
...
class GeminiBackend(PHIDetectionBackend):
    ...
    def __init__(
        ...
        self._total_deadline_seconds = total_deadline_seconds or float(
            os.environ.get("GEMINI_TOTAL_DEADLINE_SECONDS", "300")
        )
    ...
    def detect(self, dicom_paths: list[str]) -> list[FileFinding]:
        ...
        for i, path in enumerate(dicom_paths):
            ...
            if elapsed > self._total_deadline_seconds:
                log.warning(
                    "Gemini PHI detection deadline (%.0fs) exceeded after %d/%d files — "
                    "returning partial results",
                    self._total_deadline_seconds, i, len(dicom_paths),
                )
                break
