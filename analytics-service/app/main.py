...
FastAPI service that runs neuroimaging analysis tools (FreeSurfer, FSL, ANTs, SPM)
on BIDS-converted NIfTI data. Called asynchronously from the Go API after BIDS
conversion completes.

Endpoints:
  GET  /healthz         -- liveness + backend availability
  POST /analyze         -- run analytics on a BIDS study (synchronous)
...
from . import config
from .backends.base import AnalyticsBackend, AnalyticsResult
...
app = FastAPI(title="AEGIS Neuroimaging Analytics Service", version="0.1.0")
...
def analyze(req: AnalyzeRequest) -> AnalyzeResponse:
    ...
    for backend in backends:
        ...
        try:
            result = backend.analyze(req.bids_dir, output_dir, req.study_uid)
            results.append(
                ToolResultModel(
                    tool=result.tool,
