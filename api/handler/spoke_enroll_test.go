...
func makeSpokeCSR(t *testing.T, cn string) string {
	t.Helper()
...
func TestEnrollSpoke_HappyPath(t *testing.T) {
	if testing.Short() {
	...
	body, _ := json.Marshal(map[string]string{
		"token":   raw,
		"csr_pem": makeSpokeCSR(t, "umn-lab-spoke"),
		"site_id": "umn-lab-spoke",
	})
	req := httptest.NewRequest(http.MethodPost, "/api/spokes/enroll", strings.NewReader(string(body)))
	rr := httptest.NewRecorder()
	srv.EnrollSpoke(rr, req)
...
func TestEnrollSpoke_DisabledReturns503(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	db := testutil.TestDB(t)
	srv := testutil.TestServer(t, db)
	// No SetSpokeCA called — spokeCA stays nil.

	body, _ := json.Marshal(map[string]string{"token": "x", "csr_pem": makeSpokeCSR(t, "x")})
	req := httptest.NewRequest(http.MethodPost, "/api/spokes/enroll", strings.NewReader(string(body)))
	rr := httptest.NewRecorder()
	srv.EnrollSpoke(rr, req)
...
func TestEnrollSpoke_InvalidTokenReturns401(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	db := testutil.TestDB(t)
	srv := testutil.TestServer(t, db)
	signer, _ := spoke_ca.NewEphemeral(30)
	srv.SetSpokeCA(signer)

	body, _ := json.Marshal(map[string]string{
		"token": "never-issued", "csr_pem": makeSpokeCSR(t, "x"),
	})
	req := httptest.NewRequest(http.MethodPost, "/api/spokes/enroll", strings.NewReader(string(body)))
	rr := httptest.NewRecorder()
	srv.EnrollSpoke(rr, req)
...
func TestEnrollSpoke_TokenSingleUse(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	db := testutil.TestDB(t)
	...
	tok := &model.SpokeEnrollmentToken{InstitutionID: inst.ID, ExpiresAt: time.Now().UTC().Add(1 * time.Hour)}
	require.NoError(t, model.CreateSpokeEnrollmentToken(context.Background(), db, tok, hash))

	body, _ := json.Marshal(map[string]string{"token": raw, "csr_pem": makeSpokeCSR(t, "x")})
	req := httptest.NewRequest(http.MethodPost, "/api/spokes/enroll", strings.NewReader(string(body)))
	rr := httptest.NewRecorder()
	srv.EnrollSpoke(rr, req)
	...
	req2 := httptest.NewRequest(http.MethodPost, "/api/spokes/enroll", strings.NewReader(string(body)))
	rr2 := httptest.NewRecorder()
	srv.EnrollSpoke(rr2, req2)
	assert.Equal(t, http.StatusUnauthorized, rr2.Code)
}

func TestEnrollSpoke_InvalidCSR(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	db := testutil.TestDB(t)
