...
func TestRedeemEnrollmentToken_HappyPath(t *testing.T) {
	...
	redeemed, err := model.RedeemSatelliteEnrollmentToken(context.Background(), db, raw)
	require.NoError(t, err)
...
func TestRedeemEnrollmentToken_DoubleRedemptionRejected(t *testing.T) {
	...
	_, err := model.RedeemSatelliteEnrollmentToken(context.Background(), db, raw)
	require.NoError(t, err)
	_, err = model.RedeemSatelliteEnrollmentToken(context.Background(), db, raw)
...
func TestRedeemEnrollmentToken_UnknownRejected(t *testing.T) {
	...
	db := testutil.TestDB(t)
	_, err := model.RedeemSatelliteEnrollmentToken(context.Background(), db, "never-issued")
	assert.ErrorIs(t, err, model.ErrEnrollmentTokenInvalid)
}
...
func TestRevokeEnrollmentTokenBlocksRedemption(t *testing.T) {
	...
	require.NoError(t, model.CreateSatelliteEnrollmentToken(context.Background(), db, tok, hash))
	require.NoError(t, model.RevokeSatelliteEnrollmentToken(context.Background(), db, tok.ID))
...
func TestListEnrollmentTokens(t *testing.T) {
	...
	tokens, err := model.ListSatelliteEnrollmentTokens(context.Background(), db, inst.ID)
	require.NoError(t, err)
